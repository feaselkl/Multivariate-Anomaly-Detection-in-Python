...
from src.app.models.multivariate2 import *
import pandas as pd
import pytest
...
@pytest.mark.parametrize("df_input, sensitivity_score, number_of_anomalies", [
    (sample_input, 100, 8),     # Was 3 in COF-only
    (sample_input, 50, 8),      # Was 3 in COF-only
    (sample_input, 40, 8),      # Was 3 in COF-only
    (sample_input, 25, 8),      # Was 3 in COF-only
    (sample_input, 5, 5),       # Was 3 in COF-only
    (sample_input, 1, 2),       # Was 3 in COF-only
    (sample_input, 0, 0),
])
...
@pytest.mark.parametrize("df_input, max_fraction_anomalies, number_of_anomalies", [
    (sample_input, 1.0, 8),     # Was 3 in COF-only
    (sample_input, 0.8, 8),     # Was 3 in COF-only
    (sample_input, 0.6, 8),     # Was 3 in COF-only
    (sample_input, 0.4, 8),     # Was 3 in COF-only
    (sample_input, 0.2, 8),     # Was 3 in COF-only
    (sample_input, 0.1, 8),     # Was 3 in COF-only
    (sample_input, 0.01, 1),
])
